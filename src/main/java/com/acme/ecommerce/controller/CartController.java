...
public class CartController {
	...
	public RedirectView addToCart(@ModelAttribute(value="productId") long productId, @ModelAttribute(value="quantity") int quantity,
		...
		if (addProduct != null) {
				...
				for (ProductPurchase pp : purchase.getProductPurchases()) {
					if (pp.getProduct() != null) {
						if (pp.getProduct().getId().equals(productId)) {
							...
							if ((pp.getQuantity() + quantity) > PRODUCT_MAX ) {
								redirectAttributes.addFlashAttribute("flash",
										new FlashMessage("Exceed Stock", FlashMessage.Status.FAILURE));
			...
			if (!productAlreadyInCart) {
				...
				if ((newProductPurchase.getQuantity()) > PRODUCT_MAX ) {
					redirectAttributes.addFlashAttribute("flash",
							new FlashMessage("Exceed Stock", FlashMessage.Status.FAILURE));
		...
		redirectAttributes.addFlashAttribute("flash",
				new FlashMessage("Add success", FlashMessage.Status.SUCCESS));
		return redirect;
	}

	@RequestMapping(path="/update", method = RequestMethod.POST)
	public RedirectView updateCart(@ModelAttribute(value="productId") long productId,
				       @ModelAttribute(value="newQuantity") int newQuantity, RedirectAttributes redirectAttributes) {
		...
		redirectAttributes.addFlashAttribute("flash",
				new FlashMessage("Update success", FlashMessage.Status.SUCCESS));
		return redirect;
	}
	...
	public RedirectView removeFromCart(@ModelAttribute(value="productId") long productId, RedirectAttributes redirectAttributes) {
		...
		redirectAttributes.addFlashAttribute("flash",
				new FlashMessage("Remove success", FlashMessage.Status.SUCCESS));
	...
	public RedirectView emptyCart(RedirectAttributes redirectAttributes) {
		...
		redirectAttributes.addFlashAttribute("flash",
				new FlashMessage("Empty success", FlashMessage.Status.SUCCESS));
